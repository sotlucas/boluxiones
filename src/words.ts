...
import { loadGameState, clearGameState, getGameDateString } from "./gameStorage"
...
export function useGroupings(date: Date) {
  ...
  useEffect(() => {
    // Clear saved game state if it's for a different date
    const currentDateString = getGameDateString(date);
    const savedState = loadGameState();
    if (savedState && savedState.date !== currentDateString) {
      clearGameState();
    }
